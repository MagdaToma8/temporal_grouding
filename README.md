...
## Project status and handover notes

This repo extends the original paper's relevance feedback methods (PRF, AFS, GRF) from text-to-**image** retrieval to text-to-**video** retrieval, using MSR-VTT. This section is a quick-orientation summary for anyone picking this project up -- what's done, what's implemented-but-unfinished, and what's a deliberate deferral, each pointing at the relevant section below for full detail.

**Done and documented with results:**
* Video backbone: two architectures tried (CLIP mean-pooling, then ViCLIP-B), zero-shot and fine-tuned, plus a dedicated investigation into whether either genuinely uses temporal/motion information rather than just more visual samples. See "Video backbone fine-tuning" through "Video backbone, take two: ViCLIP-B". Best backbone: fine-tuned ViCLIP-B, 44.7% hits@1.
* PRF on video: worked with no code changes needed, small/mixed effect (slightly positive zero-shot, slightly negative fine-tuned). See "Relevance feedback on video: PRF".
* AFS on video, global-mode: built from scratch (dataset class, video-aware collator, backbone glue code), trained, evaluated -- underperforms baseline, with a diagnosed likely cause (an information-starved, single-vector-per-video representation). See "Toward AFS on video".
* GRF on video: built a video-captioning pipeline (LLaVA-NeXT-Video-7B, fed all sampled frames per video), generated captions for the test set, evaluated -- also underperforms baseline, though less severely than AFS. See "GRF on video".

**Implemented and verified, but the training run itself was never completed -- the main unfinished piece:**
* **Local-mode AFS.** The planned follow-up meant to test whether AFS's poor global-mode result was caused by information starvation (one blurry vector per video) or something more fundamental -- giving the network genuine per-patch/per-token detail instead. Concretely:
  * DONE: `ViCLIPWrapper` now exposes real per-patch/per-token local features (previously a placeholder duplicate of the pooled embedding) -- reimplemented directly from ViCLIP's own forward pass and numerically verified bit-exact against the original pooled output.
  * DONE: confirmed no re-run of `run_embeddings_and_retrieval.py` is needed -- AFS training re-encodes raw frames at train time, so switching global->local mode is purely a config change.
  * DONE: resume infrastructure added to `train_summarizer.py` (`--run_name`, `--resume_checkpoint`, `save_last=True`), since a full local-mode run is estimated at multiple days -- too long for one SLURM job -- so `job_train_afs_local_viclip.slurm` is designed to be resubmitted repeatedly, resuming from the last checkpoint each time.
  * **UNCONFIRMED**: `batch_size: 16` in `configs/msrvtt/viclip_local_summarizer.yaml` was chosen by extrapolating from batch_size=1/8 memory measurements, but the debug job meant to confirm it actually works on real hardware accidentally re-ran a stale, uncommitted-pull batch_size=8 config -- so 16 has **not** actually been verified yet.
  * **NOT STARTED**: the real training run (`job_train_afs_local_viclip.slurm`) was never submitted. No local-mode AFS checkpoint exists, so there is no result to compare against baseline/PRF/AFS-global/GRF.
  * **Concrete next steps**: (1) re-run `job_debug_afs_local_viclip.slurm` after confirming `git pull` actually updated the config on the cluster (`cat configs/msrvtt/viclip_local_summarizer.yaml` should show `batch_size: 16`), and check it doesn't OOM; (2) `sbatch job_train_afs_local_viclip.slurm`, then keep resubmitting the same command whenever it hits the time limit, until early stopping triggers or `max_epochs` (30) is reached; (3) evaluate the resulting checkpoint the same way the global-mode one was evaluated (`retrieval_pipeline.py --feedback_aggregation attentive_summarizer`, pointing `--experiment_config` at `configs/msrvtt/viclip_local_summarizer.yaml` and `--summarizer_checkpoint` at the trained checkpoint); (4) add the result to the comparison table in "Local-mode AFS" below with the same style of honest analysis used throughout this README -- does it beat global-mode AFS's 41.5% hits@1? Does it beat the 44.7% no-feedback baseline?

**Investigated and deliberately deferred (not a gap -- a documented scope decision):**
* A second video-text dataset (VATEX), to mirror how the original paper used two image datasets (COCO + Flickr30k). Blocked on YouTube bot-detection at the ~29k-clip scale needed. Fully documented -- including what was built and verified before hitting the blocker -- in "VATEX dataset (investigated, not currently used)", so it isn't re-investigated from scratch if revisited.

**Mentioned as a future direction but not attempted:** a tubelet-based backbone beyond ViCLIP-B (e.g. VideoMAE), raised in "Does temporal information actually help?" -- deprioritized once ViCLIP-B's own temporal-attention evidence was judged sufficient to proceed with relevance feedback experiments instead.

## Applications

Text-to-video retrieval with relevance feedback has a natural real-world use case: **finding specific moments inside a large video library from a plain-language description**, then refining the search interactively instead of hoping the first query nails it. Concrete examples:

* **Scene search in film/TV archives.** A production company, streaming platform, or archive team searching a large catalog for a specific kind of moment -- e.g. *"someone hitting someone else"*, *"a car chase through a city"*, *"a character jumping off a building"* -- without needing scene-by-scene manual tagging. This project's own temporal/static caption analysis (see "Does temporal information actually help?") is directly relevant here: queries like these are exactly what that analysis labeled "temporal-dependent" (its own examples included *"a bus crashes into a car"* and *"...jumps off"*) -- and empirically, this category already retrieves *more* reliably than generic static descriptions in every setup tested, both because such moments are visually distinctive and because ViCLIP-B's spatiotemporal attention shows measurable evidence of actually using motion specifically for this category.
* **Content moderation / compliance review.** Searching moderation queues or archives for specific depicted actions (e.g. violence, unsafe behavior) by natural-language description, rather than relying solely on manual review or pre-existing metadata tags.
* **Sports highlight retrieval.** Finding specific play types (*"a diving catch"*, *"a red card foul"*) across hours of raw match footage.
* **Surveillance/security footage review.** Searching long-running camera archives for a described event (*"someone climbing a fence"*) instead of manually scrubbing through hours of recordings.
* **E-learning / lecture archive search.** Finding the specific moment in a long lecture or training video where a described demonstration or topic occurs.

Relevance feedback specifically (the part of this project beyond plain retrieval) matters most when the first search isn't quite right: a query like *"a person falling"* might return a mix of genuine falls, someone sitting down, and an object dropping in the top results -- relevance feedback, in principle, lets the system learn from which of those results actually look right and refine the search, rather than requiring the user to hand-craft a better query. **A genuinely useful, cautionary finding from this project for anyone building such a system**: none of the three feedback mechanisms tried (PRF, AFS-global, GRF) clearly beat plain retrieval on MSR-VTT (see the results tables throughout this README). Relevance feedback is not automatically beneficial for video the way the original paper showed it to be for images, and would need further validation -- or the richer local-mode AFS signal this project didn't finish evaluating (see "Project status" above) -- before being deployed as default behavior in a real product.
...
**Compute cost, and how batch size was tuned.** Each retrieved video is now 1,569 vision tokens (vs. one pooled vector), and with `top_k_feedback=5` retrieved videos plus the query video, a single training example attends over roughly 9,400 vision tokens -- around 30x more than any local-mode config tried in the image domain so far (`configs/flickr30k/clip_local_summarizer.yaml`, single CLIP-B/32 images, batch_size 512). `--debug` runs (2 fixed train/val batches, `job_debug_afs_local_viclip.slurm`) were used to size this without risking a wasted multi-hour job: `batch_size=1` used 2.45GB peak GPU memory reserved (of 48GB available) at ~2.5s/video; `batch_size=8` used 14.03GB at ~1.9s/video -- a modest per-video speedup from batching, and confirmation that even `batch_size=8` leaves a lot of headroom. Raised to `batch_size: 16` (current value in `configs/msrvtt/viclip_local_summarizer.yaml`) by linear extrapolation (~27GB estimated) for a further speedup try -- **but this has not actually been confirmed on real hardware**: the debug job intended to verify it accidentally re-ran the stale `batch_size=8` config (a `git pull` was missed on the cluster beforehand), producing an identical-looking log. Re-confirming the config actually updated before re-running the debug job is the first remaining step.

**Why training needs to be split across multiple job submissions.** Even at `batch_size=8`, a real epoch (8,500 training videos, ~1,063 steps) is estimated at ~4.4 hours -- and the global-mode AFS run (above) never actually triggered early stopping even across its full 30-epoch budget, so local-mode training could plausibly take several days total, far past the 24h SLURM time limit used everywhere else in this project. Rather than shortening `max_epochs` (which would make local-mode less comparable to global-mode's full 30-epoch run), `train_summarizer.py` gained resume support: `--run_name` (a fixed identifier so repeated submissions write to, and can resume from, the same checkpoint directory instead of a fresh timestamped one each time) and `--resume_checkpoint` (restores epoch, optimizer/scheduler state, and the early-stopping counter from a given `.ckpt`), plus `save_last=True` on the checkpoint callback so a resumable checkpoint exists after every epoch, not just when val loss improves. `job_train_afs_local_viclip.slurm` auto-detects and resumes from the last checkpoint on every submission -- the intended workflow is `sbatch job_train_afs_local_viclip.slurm`, let it run until it hits the time limit, then resubmit the identical command, repeating until early stopping triggers or `max_epochs` is reached.

**Status: implementation complete, training not yet run.** No local-mode AFS checkpoint exists yet, so there is no result to report in this section. See "Project status and handover notes" at the top of this README for the concrete next steps.